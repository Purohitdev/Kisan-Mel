...
// ✅ Extend the window object for gtag
declare global {
  interface Window {
    gtag?: (...args: any[]) => void;
  }
...
function Contact() {
  ...
  const handleConversion = () => {
    window.gtag?.('event', 'conversion', {
      send_to: 'AW-17006319518/CONVERSION_LABEL', // 🔁 Replace with actual label if provided
    });
  };
  ...
  const sendEmail = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Set current time before sending
    const timeInput = formRef.current.querySelector<HTMLInputElement>('input[name="time"]');
    if (timeInput) {
      timeInput.value = new Date().toLocaleString();
    }
    ...
    emailjs
      ...
      .then(
        (result) => {
          ...
          alert('Message sent successfully!');
          formRef.current?.reset();
          handleConversion(); // ✅ Fire Google Ads conversion
        },
        (error) => {
          console.log(error.text);
          alert('Something went wrong. Please try again.');

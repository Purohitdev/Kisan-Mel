...
const App_promo: React.FC = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (iotWhiteBoxRef.current) {
        gsap.to(iotWhiteBoxRef.current, {
          yPercent: -20,
          ease: "none",
          scrollTrigger: {
            trigger: iotWhiteBoxRef.current,
            start: "top bottom",
            end: "bottom top",
            scrub: true,
          },
  ...
  return (
    <div className="bg-[#000] text-white px-4 md:px-16 py-12 min-h-fit mx-auto rounded-2xl">
      ...
      <div className="flex flex-wrap md:flex-nowrap items-center gap-y-10 md:gap-16">
        ...
        >
          <button className="px-4 py-2 border rounded-3xl w-fit">
            {app.button[lang]}
          </button>
          <h1 className="text-2xl md:text-5xl font-bold max-w-lg mt-4">
            {app.title[lang]}
          </h1>

          <ul className="text-gray-300 max-w-lg text-lg md:text-xl mt-5 leading-normal">
            {app.features[lang].map((item: string, idx: number) => (
              <li key={idx}>{item}</li>
            ))}
          </ul>

          <h2 className="text-lg md:text-xl font-bold mt-6 text-[#7acf4b]">
            {app.platformHeading[lang]}
          </h2>

          <div className="flex flex-wrap gap-5 mt-2">
            <div className="w-full md:w-fit p-5 px-8 py-4">
              <ul className="list-disc list-inside text-[#D9D7CD] mt-3">
                {app.platformFeatures[lang].map((item: string, idx: number) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
            </div>
            <div className="w-full md:w-fit p-5 px-8 py-4">
              <ul className="list-disc list-inside text-[#D9D7CD] mt-3">
                {app.platformFeatures2[lang].map((item: string, idx: number) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
            </div>
      ...
      <div className="mt-16 flex flex-wrap md:flex-nowrap items-center gap-y-4 md:gap-10">
        <div
          ref={(el) => {
            iotWhiteBoxRef.current = el;
          }}
          className="w-full md:w-2/3"
        >
          <button className="px-4 py-2 rounded-3xl w-fit border">
            {iot.button[lang]}
          </button>
          ...
          <div className="flex flex-wrap gap-5 mt-6">
            {iot.sections.map((sec: any, i: number) => (
              <div key={i} className="w-full md:w-fit py-4">
                <h3 className="text-lg md:text-xl font-bold mb-3 text-[#7acf4b]">
                  {sec.heading[lang]}
                </h3>
                <div className="text-[#D9D7CD] p-5">
                  <ul className="list-disc list-inside mt-3 space-y-1">
                    {sec.points[lang].map((point: string, idx: number) => (
                      <li key={idx}>{point}</li>
                    ))}
                  </ul>
                </div>
        ...
        <div className="flex justify-center w-full md:w-1/3">
          <img
            src="/farm3.jpg"
            alt="IoT Farm System"
            className="rounded-lg shadow-lg w-[90%] max-w-[600px] h-auto"
          />
        </div>
      </div>
    </div>
